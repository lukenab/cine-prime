  ...
  ({ label, icon, rightIcon, error, onRightIconClick, className, ...props }, ref) => {
    ...
    return (
      <div className="relative">
        <div className={`
          relative flex items-center border rounded-xl transition-all duration-200 bg-white
          ${error
        ...
        `}>
          {icon && (
            <div className={`absolute left-3.5 transition-colors duration-200 ${focused ? "text-indigo-500" : "text-gray-400"}`}>
              {icon}
            </div>
          )}
          <input
            ...
            className={`
              peer w-full bg-transparent outline-none pt-5 pb-1 px-3.5
              ${icon ? "pl-10" : "pl-3.5"}
          ...
          {rightIcon && (
            <button
              ...
              className="absolute right-3.5 text-gray-400 hover:text-gray-600 transition-colors duration-150 focus:outline-none"
            >
              {rightIcon}
            </button>
          )}
