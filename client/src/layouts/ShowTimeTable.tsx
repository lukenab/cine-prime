...
export function ShowtimeTable({
  ...
  const [confirmDeleteShowtime, setConfirmDeleteShowtime] = useState<ShowtimeResponse | null>(null);
  ...
  const handleConfirmDelete = () => {
    if (!confirmDeleteShowtime) return;
    onDelete(confirmDeleteShowtime.showtimeId);
    ...
    <>
      ...
      {confirmDeleteShowtime && (
        <div
          ...
          onClick={() => setConfirmDeleteShowtime(null)}
        >
          <div
          ...
          >
            ...
            <p style={{ fontSize: "13px", color: "var(--text-sub)", textAlign: "center", marginBottom: "20px" }}>
              ...
              <span style={{ fontWeight: 600, color: "var(--text-main)" }}>{confirmDeleteShowtime.movieName}</span>?
              <br />
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b" style={{ borderColor: "var(--border-color)", backgroundColor: "rgba(128,128,128,0.04)" }}>
                ...
                <th className="px-5 py-3.5 text-left"><span style={{ color: "var(--text-sub)", fontSize: "11px", fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase" }}>Cinema</span></th>
                ...
                <th className="px-5 py-3.5 text-left"><span style={{ color: "var(--text-sub)", fontSize: "11px", fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase" }}>Base Price</span></th>
              ...
              {pageItems.length === 0 ? (
                ...
                pageItems.map((item) => (
                  <tr key={item.showtimeId} className="border-b last:border-none hover-row transition-colors" style={{ borderColor: "var(--border-color)" }}>
                    ...
                    <td className="px-5 py-3.5">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${getStatusBadgeClass(item.status)}`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${getStatusDotClass(item.status)}`} />
                        {item.status}
                      </span>
                    </td>
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <button onClick={() => setConfirmDeleteShowtime(item)} className="w-8 h-8 rounded-lg flex items-center justify-center transition-colors action-btn text-rose-400 hover:text-rose-500">
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </td>
