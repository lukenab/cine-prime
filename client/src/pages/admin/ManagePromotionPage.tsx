...
import { Archive, CalendarClock, CheckCircle2, ChevronLeft, ChevronRight, CircleAlert, CircleCheck, Eye, Pause, Pencil, Play, Plus, RefreshCw, RotateCcw, Search, Send, Tag } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { promotionApi, type PromotionAvailabilityStatus, type PromotionSummary, type PromotionStatus } from "../../api/promotionApi";
...
const emptyCounts = { total: 0, draft: 0, pendingApproval: 0, approved: 0, rejected: 0, active: 0, paused: 0, archived: 0, approvedOrScheduled: 0, activeNow: 0 };
...
function statusStyle(status: PromotionStatus) {
  const styles: Record<PromotionStatus, { label: string; color: string; bg: string }> = {
    ...
    ACTIVE: { label: "Approved", color: "#2563eb", bg: "rgba(37,99,235,.12)" },
    PAUSED: { label: "Approved", color: "#2563eb", bg: "rgba(37,99,235,.12)" },
    ARCHIVED: { label: "Not available", color: "#94a3b8", bg: "rgba(148,163,184,.12)" },
  };
  return styles[status];
}

function filterStatusLabel(status: PromotionStatus) {
  if (status === "APPROVED") return "Approved · not activated";
  if (status === "ACTIVE") return "Approved · activated";
  if (status === "PAUSED") return "Approved · paused";
  return statusStyle(status).label;
}

function availabilityOf(promotion: PromotionSummary): PromotionAvailabilityStatus {
  ...
  if (promotion.globalUsageLimit != null && (promotion.activeReservationCount + (promotion.committedUsageCount ?? 0)) >= promotion.globalUsageLimit) return "QUOTA_EXHAUSTED";
  return "ACTIVE";
}

function availabilityStyle(status: PromotionAvailabilityStatus) {
  const styles: Record<PromotionAvailabilityStatus, { label: string; color: string; bg: string }> = {
    NOT_AVAILABLE: { label: "Not available", color: "#64748b", bg: "rgba(100,116,139,.12)" },
    SCHEDULED: { label: "Scheduled", color: "#2563eb", bg: "rgba(37,99,235,.12)" },
    ACTIVE: { label: "Active", color: "#059669", bg: "rgba(16,185,129,.12)" },
    PAUSED: { label: "Paused", color: "#d97706", bg: "rgba(245,158,11,.12)" },
    ENDED: { label: "Ended", color: "#64748b", bg: "rgba(100,116,139,.12)" },
    QUOTA_EXHAUSTED: { label: "Quota exhausted", color: "#dc2626", bg: "rgba(220,38,38,.10)" },
...
export default function ManagePromotionPage() {
  ...
  const loadPromotions = useCallback(async () => {
    ...
    try {
      ...
      const responseCounts = result.counts ?? emptyCounts;
      setCounts({
        ...emptyCounts,
        ...responseCounts,
        approvedOrScheduled: responseCounts.approvedOrScheduled ?? responseCounts.approved ?? 0,
        activeNow: responseCounts.activeNow ?? responseCounts.active ?? 0,
      });
    } catch {
      if (requestId !== requestSequence.current) return;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {[
          { label: "Needs approval", value: counts.pendingApproval, helper: "Submitted for commercial review", icon: CalendarClock, tone: "#d97706", bg: "rgba(245,158,11,.12)" },
          { label: "Changes requested", value: counts.rejected, helper: "Returned to the campaign owner", icon: CircleAlert, tone: "#dc2626", bg: "rgba(220,38,38,.10)" },
          { label: "Approved / scheduled", value: counts.approvedOrScheduled, helper: "Approved offers not active yet", icon: CheckCircle2, tone: "#2563eb", bg: "rgba(37,99,235,.10)" },
          { label: "Active now", value: counts.activeNow, helper: "Currently available to customers", icon: CircleCheck, tone: "#059669", bg: "rgba(16,185,129,.10)" },
        ].map(({ label, value, helper, icon: Icon, tone, bg }) => (
          <div key={label} className="flex min-h-28 items-start justify-between gap-4 rounded-2xl border p-5" style={{ background: "var(--bg-card)", borderColor: "var(--border-color)" }}>
            <div><p className="text-xs" style={{ color: "var(--text-sub)" }}>{label}</p><p className="mt-2 text-3xl font-semibold" style={{ color: "var(--text-main)" }}>{Number.isFinite(value) ? value : 0}</p><p className="mt-1 text-xs" style={{ color: "var(--text-sub)" }}>{helper}</p></div>
            <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl" style={{ color: tone, background: bg }}><Icon size={18} /></span>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <select value={status} onChange={(event) => { setStatus(event.target.value as PromotionStatus | ""); setPage(0); }} className="h-11 rounded-xl border px-4 text-sm outline-none" style={{ color: "var(--text-main)", borderColor: "var(--border-color)", background: "var(--bg-card)" }}>
          ...
          {(["DRAFT", "PENDING_APPROVAL", "APPROVED", "REJECTED", "ACTIVE", "PAUSED", "ARCHIVED"] as PromotionStatus[]).map(value => <option key={value} value={value}>{filterStatusLabel(value)}</option>)}
        </select>
      ...
      <div className="overflow-hidden rounded-2xl border" style={{ background: "var(--bg-card)", borderColor: "var(--border-color)" }}>
        <div className="overflow-x-auto">
          <table className="w-full min-w-[1050px] table-fixed text-left">
            <thead className="border-b text-xs uppercase tracking-wider" style={{ color: "var(--text-sub)", borderColor: "var(--border-color)" }}><tr><th className="w-[29%] px-5 py-4">Promotion</th><th className="w-[13%] px-5 py-4">Benefit</th><th className="w-[19%] px-5 py-4">Valid window</th><th className="w-[13%] px-5 py-4">Workflow</th><th className="w-[13%] px-5 py-4">Availability</th><th className="w-[13%] px-2 py-4 text-right">Actions</th></tr></thead>
            <tbody>
              {loading ? <tr><td colSpan={6} className="px-5 py-16 text-center text-sm" style={{ color: "var(--text-sub)" }}>Loading promotions...</td></tr> : promotions.length === 0 ? <tr><td colSpan={6} className="px-5 py-16 text-center text-sm" style={{ color: "var(--text-sub)" }}>No promotions found.</td></tr> : promotions.map((promotion) => {
                const badge = statusStyle(promotion.status);
                const availability = availabilityStyle(availabilityOf(promotion));
                ...
                return <tr key={promotion.promotionId} className="border-b last:border-0 hover:bg-blue-500/[.025]" style={{ borderColor: "var(--border-color)" }}>
                  <td className="px-5 py-4"><div className="flex items-center gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-blue-500/10 text-blue-500"><Tag size={18} /></span><div className="min-w-0"><button type="button" onClick={() => navigate(`/admin/promotions/${promotion.promotionId}`)} className="block max-w-full truncate text-left font-medium hover:text-blue-500" style={{ color: "var(--text-main)" }}>{promotion.name}</button><p className="truncate text-xs" style={{ color: "var(--text-sub)" }}>{promotion.code}</p></div></div></td>
                  ...
                  <td className="px-5 py-4"><span className="rounded-full px-2.5 py-1 text-xs font-semibold" style={{ color: availability.color, background: availability.bg }}>{availability.label}</span></td>
                  <td className="w-[13%] px-2 py-4 text-right">
                    <RowActions
                      ariaLabel={`Actions for ${promotion.name}`}
                      busy={workingId === promotion.promotionId}
                      actions={[
                        { key: "view", label: "View details", icon: Eye, onSelect: () => navigate(`/admin/promotions/${promotion.promotionId}`) },
                        { key: "edit", label: "Edit draft", icon: Pencil, onSelect: () => navigate(`/admin/promotions/edit/${promotion.promotionId}`), hidden: !editable, separatorBefore: true },
                        { key: "submit", label: "Submit for approval", icon: Send, onSelect: () => setPendingAction({ promotion, action: "submit" }), hidden: !canSubmit, separatorBefore: !editable },
                        { key: "approve", label: "Approve", icon: CheckCircle2, onSelect: () => setPendingAction({ promotion, action: "approve" }), hidden: !canDecide, separatorBefore: true },
                        { key: "reject", label: "Return for changes", icon: RotateCcw, onSelect: () => setPendingAction({ promotion, action: "reject" }), hidden: !canDecide, destructive: true },
                        { key: "activate", label: promotion.status === "PAUSED" ? "Resume promotion" : "Activate promotion", icon: Play, onSelect: () => setPendingAction({ promotion, action: "activate" }), hidden: !canActivate, separatorBefore: true },
                        { key: "pause", label: "Pause promotion", icon: Pause, onSelect: () => setPendingAction({ promotion, action: "pause" }), hidden: !canPause, separatorBefore: true },
                        { key: "archive", label: "Archive promotion", icon: Archive, onSelect: () => setPendingAction({ promotion, action: "archive" }), hidden: !canArchive, destructive: true, separatorBefore: true },
                      ] satisfies RowAction[]}
                    />
                  </td>
                </tr>;
