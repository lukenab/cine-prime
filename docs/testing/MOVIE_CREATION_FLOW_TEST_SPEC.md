...
| Import (TMDB) | Nhập dữ liệu | Có 2 nghĩa khác nhau cần phân biệt rõ: (a) endpoint `POST /api/movies/tmdb/import` — tạo movie trực tiếp từ TMDB, **hiện không có UI gọi**; (b) hành vi UI thật — dùng TMDB preview để **prefill** form Manual Editor rồi lưu qua `createMovieV2` như bình thường. |
...
| 5 | ADMIN/EMPLOYEE | Bấm "Save Draft" lần đầu (route `/new/manual`, chưa có `movieId`) | `POST /api/movies` (`createMovieV2`, qua `buildMoviePayload()` → `CreateMovieRequest`) | `200`, `result` = `MovieV2` mới với `status=DRAFT` | Insert `movie` + (nếu có) `movie_translation`, `movie_cast`; ghi 1 dòng `movie_action_log` ("Created movie: ...") | `409 MOVIE_ALREADY_EXISTS` nếu trùng `originalTitle`; `404 *_NOT_FOUND` nếu genre/format/ageRating/company/person ID sai |
...
- **Khi nào movie record được tạo?** Chỉ khi user bấm "Save Draft" trên `MovieEditorPage` (`createMovieV2`) — không phải ngay khi "Use this movie".
...
- **Cách tránh null overwrite?** `UpdateMovieRequest` dùng `NullValuePropertyMappingStrategy.IGNORE` — field không gửi/`null` không ghi đè giá trị hiện có. Riêng với luồng TMDB-assisted hiện tại (đi qua `createMovieV2` thường), toàn bộ field trong `CreateMovieRequest` được set 1 lần lúc tạo — không có khái niệm "null overwrite" cho creation.
...
|---|---|---|
| Load dữ liệu | `GET /api/movies/{id}` → `movieToForm()` map `MovieV2` sang `FormState` | |
...
|---|---|---|---|---|---|
| Tạo draft, chặn trùng title | `MovieService.createMovie` | `MovieEditorPage` → `createMovieV2` | API-MOV-001, API-CREATE-008 | UI-MOV-012 | Implemented |
...
|---|---|---|---|---|
| **#1 — TMDB one-shot import (`POST /api/movies/tmdb/import`) không có UI trigger** | `TmdbCatalogPage.tsx` "Use this movie" chỉ điều hướng + truyền `router state`, không gọi `movieApi.tmdbImport()`; `TmdbCatalogPage.test.tsx` có assertion tường minh `expect(mocks.tmdbImport).not.toHaveBeenCalled()` | Toàn bộ logic nghiệp vụ TMDB-specific (duplicate theo `tmdbId`/`imdbId` ngay từ đầu, resolve genre `selectedGenreMappings`/`createPendingGenres`/`ignoredGenres`, `confirmedRuntimeMinutes`, `confirmedAgeRatingId`) không thể demo qua UI; đường thật (qua form thường) không có các bảo vệ tương đương (ví dụ: 2 người cùng prefill 1 `tmdbId` chỉ được bắt bởi `DB UNIQUE constraint` → lỗi generic `DATA_INTEGRITY_VIOLATION` thay vì `TMDB_MOVIE_ALREADY_EXISTS`) | P1 | Quyết định rõ: (a) build UI thật gọi `/tmdb/import`, hoặc (b) chính thức deprecate endpoint đó và tài liệu hoá luồng hiện tại (prefill + manual save) là luồng chính thức duy nhất, tránh 2 luồng "TMDB import" gây nhầm lẫn khi review/demo |
...
- [ ] TMDB: "Use this movie" không gọi `/tmdb/import` (vẫn đúng theo thiết kế hiện tại, hoặc đã được thay đổi có chủ đích — cập nhật lại tài liệu này nếu có PR thay đổi hành vi)
