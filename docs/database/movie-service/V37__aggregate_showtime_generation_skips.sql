-- A skip is audit data, not a business event. Aggregate repeated normal rejections
-- so one generation run does not create thousands of rows with the same reason.

ALTER TABLE showtime_generation_skip
    ADD COLUMN IF NOT EXISTS occurrence_count INTEGER NOT NULL DEFAULT 1,
    ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW();

-- Collapse pre-V37 detailed rows. Keep one representative candidate/time/detail and
-- retain the real number of rejected candidates in occurrence_count.
